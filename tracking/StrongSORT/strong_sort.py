...
if __name__ == '__main__':
    ...
    for i, seq in enumerate(opt.sequences, start=1):
        print(f'\n>>> [{i}/{len(opt.sequences)}] Processant vídeo: {seq}')
        path_seq = join(opt.dir_dataset, seq)
        path_det = join(opt.dir_dets, seq + '.npy')
        path_save = join(opt.dir_save, seq + '.txt')

        print(f' - Ruta imatges: {path_seq}')
        print(f' - Deteccions .npy: {path_det}')
        print(f' - Resultat es guardarà a: {path_save}')
        run(
